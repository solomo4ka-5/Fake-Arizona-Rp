...
<i  class="\" aria-hidden="true" style="font-size: 40px"></i>
				...
				<script>
				                                        $(document).ready(function () {
				                                            ...
				                                            $("#inputNumber").on("input keypress keyup propertychange", function (event) {
				                                                if (!check_number()) {
				                                                    $("#numalert").show();
				                                            ...
				                                            function check_limit_min() {
				                                                var min_rub = <?= $params["MIN_QIWI_BTC"]; ?>;
				                                                var rub = $("#inputAmountRub").val();
				                                                return !(rub < min_rub)
				                                            }

				                                            function check_limit_max() {
				                                                var max_rub = <?= $params["MAX_QIWI_BTC"]; ?>;
				                                                var rub = $("#inputAmountRub").val();
				                                                return !(rub > max_rub)
				                                            }
				                                            ...
				                                            function check_form() {
				                                                if (check_limit_min() && check_limit_max() && check_addr() && check_number()) {
					...
					<div class="col-sm-3">
						<div class="visible-xs">
							<nav class="navbar navbar-default">
								...
								<span class="label bg-danger-darken" style="position:relative;top:10px;z-index:3"></span> <a class="list-group-item active" href="/">
					...
					<div class="col-sm-9">
						<section class="panel corner-flip" style="padding:10px">
							<div class="panel-heading">
								<div class="hidden-xs">
									<h3>QIWI → Биткоин</h3><small>Автоматическая покупка биткоин за QIWI</small>
							...
							<div class="panel-body">
								<form action="order_rub2btc.php" class="form" data-alignlabel="left" data-collabel="3" method="post">
												...
												<div id="rub-error">
													<div class="form-group">
														...
														<div class="input-group">
															<input oninput="this.value = this.value.replace(/[^\\d\\.0-9]/g, '')" autocomplete="off" class="form-control amount input-lg" id="inputAmountRub" name="rub_amount" placeholder="от <?= $params["MIN_QIWI_BTC"]; ?> до <?= $params["MAX_QIWI_BTC"]; ?>" type="text">
															<div class="input-group-addon">₽
															</div>
														</div>
														<ul class="parsley-error-list" id="minalert" style="display:none">
															<li class="max" style="display: list-item; color: #a94442;font-size: 11px;">Минимальный обмен – <strong><?= $params["MIN_QIWI_BTC"]; ?> RUB</strong></li>
														</ul>
														<ul class="parsley-error-list" id="maxalert" style="display:none">
															<li class="max" style="display: list-item; color: #a94442;font-size: 11px;">Лимит одного обмена – <strong><?= $params["MAX_QIWI_BTC"]; ?> RUB</strong></li>
														</ul>
													</div>
												</div>
											</div>
									...
									<div id="num-error">
										<div class="form-group">
											...
											<div>
												<input class="form-control input-lg" id="inputNumber" name="number" placeholder="+" type="text" value="">
											</div>
