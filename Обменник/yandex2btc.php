...
<font face="Open Sans" size=3>
            <strong> <a href="index.php" class="fa fa-home" > ГЛАВНАЯ</a>&nbsp
              <a href="agreement.html.html" class="fa fa-users"> ПРАВИЛА</a>&nbsp;
				...
				<script>
				                            $(document).ready(function () {
				                                ...
				                                function check_limit_min() {
				                                    var min_rub = <?= $params["MIN_CARD_BTC"]; ?>;
				                                    var rub = $("#inputAmountRub").val();
				                                    return !(rub < min_rub)
				                                }

				                                function check_limit_max() {
				                                    var max_rub = <?= $params["MAX_CARD_BTC"]; ?>;
				                                    var rub = $("#inputAmountRub").val();
				                                    return !(rub > max_rub)
				                                }
				                                ...
				                                function check_form() {
				                                    if (check_limit_min() && check_limit_max() ()) {
				                                        ...
				                                        $("#submitBtn").attr('disabled', 'disabled');
				                                            ...
				                                            $("#btcAddress").on("input keypress keyup propertychange", function (event) {
				                                                if (!check_addr()) {
				                                                    $("#btcalert").show();
				                                                    $("#btc-error").addClass('has-error');
				                                                } else {
				                                                    $("#btcalert").hide();
				                                                    $("#btc-error").removeClass('has-error');
				                                                }
				                                                check_form()
				                                            });
				                                            ...
				                                            function check_addr() {
				                                                var addr = $("#btcAddress").val();
				                                                addr = addr.replace(/\s+/g, '');
				                                                var len_max = 35;
				                                                var len_min = 26;
				                                                var reg = /[0OIl]/.test(addr);
				                                                return !(addr.length > len_max || addr.length < len_min || reg);
				                                            }


				                                            function check_form() {
				                                                if (check_limit_min() && check_limit_max() && check_addr()) {
				                                                    $("#submitBtn").removeAttr('disabled');
				                                                } else {
				                                                    $("#submitBtn").attr('disabled', 'disabled');
				                                                }
					...
					<div class="col-sm-3">
						<div class="visible-xs">
							<nav class="navbar navbar-default">
								...
								<p class="list-group-item-text hidden-xs"><small><strong><?= $kursx; ?> ₽ = 1 BTC</strong></small></p></a> <a class="cvet list-group-item" href="btc2rub.php">
                               ...
                               <span class="label bg-danger-darken" style="position:relative;top:10px;z-index:3"></span> <a class="cvet list-group-item" href="btc2yandex.php">
								...
								<p class="list-group-item-text hidden-xs"><small><strong>1 BTC = <?= $kursx; ?> ₽</strong></small></p></a>
                                <span class="label bg-danger-darken" style="position:relative;top:10px;z-index:3"></span> <a class="cvet list-group-item active" href="yandex2btc.php">
						...
						<div class="hidden-xs">
							<div class="list-group">
								<span class="label bg-danger-darken" style="position:relative;top:10px;z-index:3"></span> <a class="list-group-item" href="/">
								<h4 class="list-group-item-heading">QIWI <strong>→</strong> Биткоин</h4>
								<p class="list-group-item-text hidden-xs"><small><strong><?= $qiwi_btc; ?> ₽ = 1 BTC</strong></small></p></a> <a class="cvet list-group-item " href="btc2rub.php">
								<h4 class="list-group-item-heading">Биткоин → QIWI</h4>
								<p class="list-group-item-text hidden-xs"><small><strong>1 BTC = <?= $kursx; ?>  ₽</strong></small></p></a> 
                                 <span class="label bg-danger-darken" style="position:relative;top:10px;z-index:3"></span> <a class="cvet list-group-item" href="btc2card.php">
								<h4 class="list-group-item-heading">Биткоин → Карта</h4>
					...
					<div class="col-sm-9">
						<section class="panel corner-flip" style="padding:10px">
							<div class="panel-heading">
								<div class="hidden-xs">
									<h3>Яндекс → Биткоин</h3><small>Автоматический перевод на ваш Биткоин</small>
									<div style="float:right">
										<small>1 BTC = <?= $kursx; ?> RUB</small>
									</div>
								</div>
								<div class="visible-xs">
									<small>&nbsp;</small>
							...
							<div class="panel-body">
								<form action="order_yandex2btc" class="form" data-alignlabel="left" data-collabel="3" id="form_btcrub" method="post" name="form_btcrub">
												...
												<div id="rub-error">
													<div class="form-group">
														...
														<div class="input-group">
															<input oninput="this.value = this.value.replace(/[^\\d\\.0-9]/g, '')" autocomplete="off" class="form-control amount input-lg" id="inputAmountRub" name="rub_amount" placeholder="от <?= $params["MIN_YANDEX_BTC"]; ?> до <?= $params["MAX_YANDEX_BTC"]; ?>" type="text">
															<div class="input-group-addon">₽
															</div>
														</div>
														<ul class="parsley-error-list" id="minalert" style="display:none">
															<li class="max" style="display: list-item; color: #a94442;font-size: 11px;">Минимальный обмен – <strong><?= $params["MIN_YANDEX_BTC"]; ?> RUB</strong></li>
														</ul>
														<ul class="parsley-error-list" id="maxalert" style="display:none">
															<li class="max" style="display: list-item; color: #a94442;font-size: 11px;">Лимит одного обмена – <strong><?= $params["MAX_YANDEX_BTC"]; ?> RUB</strong></li>
														</ul>
													</div>
												</div>
											</div>
									...
									<div id="btc-error">
										<div class="form-group">
											<label class="control-label">Биткоин-адрес</label>
											<div>
												<input class="form-control input-lg parsley-validated" id="btcAddress" name="btc_address" placeholder="" type="text" value="">
											</div>
											<ul class="parsley-error-list" id="btcalert" style="display:none">
												<li class="max" style="display: list-item; color: #a94442;font-size: 11px;">Биткоин-адрес должен быть от 26 до 34 символов</li>
											</ul>
										</div>
									</div>
